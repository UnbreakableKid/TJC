...
public class NewStrategy  extends Strategy {
    ...
    public void execute() throws InterruptedException {
        ...
        while(true) {
            ...
            while(! playComplete ) {
                ...
                game.showGame();
                if(U1.length == 2){
                    System.out.println("It's a squareeeeeeee");
                    strategyP1 = setMixedStrategy(1, labelsP1, myStrategy, U1);
                    strategyP2 = setMixedStrategy(2, labelsP2, myStrategy, U2);
                }else {
                    strategyP1 = setStrategy(1, labelsP1, myStrategy);
                    strategyP2 = setStrategy(2, labelsP2, myStrategy);
                }
    ...
    public double[] setMixedStrategy(int P, String[] labels, PlayStrategy myStrategy, int[][] utilities){
        int n = labels.length;
        double[] strategy = new double[n];
        ...
        double q = 0;
        if (P == 1){
            double line1 = utilities[0][0] * p + utilities[0][2] * (1-p);
            double line2 = utilities[1][0] * p + utilities[1][2] * (1-p);

        }
        else{
            double colum1 = utilities[0][1]*q + utilities[0][3] * (1-q);
            double colum2 = utilities[1][1] * q + utilities[1][3] * (1-q);
